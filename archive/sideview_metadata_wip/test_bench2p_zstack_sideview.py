...
def test_detect_bright_candidates_returns_physical_coordinates() -> None:
    ...
    assert (candidate.z_relative_um, candidate.y_um, candidate.x_um) == (12.0, 40.0, 44.0)
    assert candidate.z_scanimage_um == 12.0


def test_detect_bright_candidates_accepts_physical_distance_and_scanimage_z() -> None:
    volume = np.zeros((25, 40, 40), dtype=np.float32)
    volume[12, 20, 22] = 100
    config = {
        "background_sigma_um": [4, 4, 4],
        "feature_sigma_um": [1, 1, 1],
        "threshold_percentile": 99.0,
        "minimum_distance_um": 4.0,
        "exclude_border_um": [3.0, 3.0, 3.0],
        "candidate_count": 1,
    }
    z_scanimage = np.arange(25) * 2.0 + 0.992
    candidates, _ = detect_bright_candidates(
        volume, (2.0, 1.0, 1.0), config, z_scanimage
    )
    assert len(candidates) == 1
    assert candidates[0].z_relative_um == 24.0
    assert candidates[0].z_scanimage_um == 24.992
